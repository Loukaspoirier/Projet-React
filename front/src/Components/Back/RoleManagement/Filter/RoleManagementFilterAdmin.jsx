...
export default function RoleManagementFilterAdmin() { // This is a component to read all roles for admin users
  ...
  const fetchData = async () => { // Create a function to fetch the signups
    try {
      setLoading(true);
      const response = await fetch("/role-management/role-admin", {
        method: "GET",
        mode: "cors",
      });
      if (!response.ok) {
