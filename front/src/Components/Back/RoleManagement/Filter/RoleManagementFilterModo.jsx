...
export default function RoleManagementFilterModo() { // This is a component to read all roles for modo users
  ...
  const fetchData = async () => {  // Create a function to fetch the signups
    try {
      setLoading(true);
      const response = await fetch("/role-management/role-modo", {
        method: "GET",
        mode: "cors",
      });
      if (!response.ok) {
