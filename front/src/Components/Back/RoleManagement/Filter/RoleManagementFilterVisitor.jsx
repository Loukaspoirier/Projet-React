...
export default function RoleManagementFilterVisitor() { // This is a component to read all roles for visitor users
  ...
  const fetchData = async () => { // Create a function to fetch the signups
    try {
      setLoading(true);
      const response = await fetch("/role-management/role-visitor", {
        method: "GET",
        mode: "cors",
      });
      if (!response.ok) {
